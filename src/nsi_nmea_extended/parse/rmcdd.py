...
def parse(s):
    ...
    if len(s[3].split('.')[0]) != 2:
        raise ValueError(f"Invalid latitude format (correct format : dd.dd- ; given value : {s[3]}).")
    if len(s[5].split('.')[0]) != 2:
        raise ValueError(f"Invalid longitude format (correct format : dd.dd- ; given value : {s[5]}).")
